...
class UserSlimBPRRecommender(BaseUserSimilarityMatrixRecommender, Incremental_Training_Early_Stopping):
    RECOMMENDER_NAME = "UserSlimBPRRecommender"
    def __init__(self, URM_train, verbose=True):
        super(UserSlimBPRRecommender, self).__init__(URM_train, verbose=verbose)
        self.W_sparse = None
    ...
    def fit(self, epochs=300,
            ...
            sgd_mode='adagrad', gamma=0.995, beta_1=0.9, beta_2=0.999,
            **earlystopping_kwargs):
        calculator = SLIM_BPR_Cython(self.URM_train.T, verbose=self.verbose)
        calculator.fit(positive_threshold_BPR=positive_threshold_BPR, train_with_sparse_weights=train_with_sparse_weights,
                       allow_train_with_sparse_weights=allow_train_with_sparse_weights, symmetric=symmetric, epochs=epochs,
                       random_seed=random_seed, lambda_i=lambda_i, lambda_j=lambda_j, learning_rate=learning_rate,
                       sgd_mode=sgd_mode, gamma=gamma, beta_1=beta_1, beta_2=beta_2, topK = topK, **earlystopping_kwargs)
        self.W_sparse = sps.csr_matrix(calculator.W_sparse)
