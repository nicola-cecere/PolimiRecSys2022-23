...
class SSLIMElasticNetRecommender(BaseItemSimilarityMatrixRecommender):
    """
    Train a Sparse Linear Methods (SLIM) item similarity model.
    NOTE: ElasticNet solver is parallel, a single intance of SLIM_ElasticNet will
          make use of half the cores available
    ...
    RECOMMENDER_NAME = "SSLIMElasticNetRecommender"

    def __init__(self, URM_train, ICM, verbose = True):
        super(SSLIMElasticNetRecommender, self).__init__(URM_train, verbose = verbose)
        self.ICM = ICM

    @ignore_warnings(category=ConvergenceWarning)
    def fit(self, l1_ratio=0.1, alpha = 1.0, positive_only=True, topK = 100, icm_coeff = 1):
        ...
        #for i in range(self.nof_icms):
        self.URM_train = sps.vstack([self.URM_train * self.icm_coeff, self.ICM.T * (1-self.icm_coeff)])
