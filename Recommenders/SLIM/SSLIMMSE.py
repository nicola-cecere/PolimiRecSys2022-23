...
import numpy as np
from Recommenders.SLIM.Cython.SLIM_MSE_Cython_Epoch import train_multiple_epochs
...
class SSLIMRMSERecommender(Recommender):
    ...
    RECOMMENDER_NAME = "SSLIMRMSERecommender"
    def __init__(self, URM: sp.csr_matrix, ICM, exclude_seen=True, add_side_info=True):
        super().__init__(URM, ICM, exclude_seen)
        self.A = self.URM.copy()
        self.add_side_info = add_side_info

    def fit(self, learning_rate=1e-5, beta=1, epochs=80, l1_reg=3e-5):
        self.learning_rate = learning_rate
        self.beta = beta
        self.epochs = epochs
        self.l1_reg = l1_reg

        if self.add_side_info:
