...
class UserRp3Recommender(BaseUserSimilarityMatrixRecommender):
    RECOMMENDER_NAME = "UserRP3betaRecommender"
    def __init__(self, URM_train, verbose=True):
        super(UserRp3Recommender, self).__init__(URM_train, verbose=verbose)
        self.W_sparse = None

    def fit(self, topK=100, alpha=1., beta=0.6, min_rating=0, implicit=False, normalize_similarity=False):
        calculator = RP3betaRecommender(self.URM_train.T, verbose=True)
        ...
        print(type(calculator.W_sparse))
