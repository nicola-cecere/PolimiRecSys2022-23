 ...
 * matrix of either 0s or 1s
 */
public class WithBinaryValues extends ScoreComputer {

    public WithBinaryValues(List<Interaction> interactions, List<Item> items) {
        ...
        for(Interaction interaction : interactions){
            ...
            if(last_action != null && last_action.getIduser() == user_id && last_action.getIditem() == item_id){ //same user-item couple -> adding the score
                ...
                if(last_action != null){ //user-item couple has changed: saving the old one
                    ...
                    sb.append(last_action.getIduser()).append(",").append(last_action.getIditem()).append(",").append(toBinary(last_action.getScore())).append("\n");
                    bw.append(sb);
    ...
    private float toBinary(float v){
        return v == 0 ? v : 1;
    }
}
